...
def migrate_athlete_profile(conn):
    """Add missing columns to athlete_profile."""
    ...
    for col_name, col_type in new_columns:
        if col_name not in existing:
            print(f"  Adding athlete_profile.{col_name}")
            conn.execute(f"ALTER TABLE athlete_profile ADD COLUMN {col_name} {col_type}")
            added += 1
    return added

def migrate_activity_metrics(conn):
    """Add missing columns to activity_metrics."""
    ...
    for col_name, col_type in new_columns:
        if col_name not in existing:
            print(f"  Adding activity_metrics.{col_name}")
...
def main():
    ...
    print("\nMigrating athlete_profile...")
    added1 = migrate_athlete_profile(conn)
    
    print("\nMigrating activity_metrics...")
    added2 = migrate_activity_metrics(conn)
    
    conn.commit()
    conn.close()
    
    print(f"\nDone! Added {added1 + added2} columns total.")
    return 0
